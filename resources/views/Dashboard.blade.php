...
<div class="col-md-12">
        ...
        <div class="col-md-9 col-md-offset-2">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <a href="/dashboard/create" style="margin:40px;" class="btn btn-dark pull-right">انشاء خبر</a>
                    @if(count($posts) > 0)
                        <table class="table table-striped">
                            ...
                            @foreach($posts as $post)
                                <tr style="color:black;" >
                                    <td>
                                    <a style="color:black; font-size:19px;" href="dashboard/{{$post->id}}">
                                    <p >{{$post->title}}</p></a>
                                    </td>

                        @if(auth()->user()->id == $post->user_id )

                @if($post->isAccepted() != 1 || auth()->user()->isEdiAd())
                                    <td>
                                    <a style="color:black;" href="dashboard/{{$post->id}}/edit" class="btn btn-secondary">تعديل</a>
                                    </td>
                        @endif

                                    <td>

                                        {!!Form::open(['action' => ['UserDashboardController@destroy', $post->id], 'method' => 'POST', 'class' => 'pull-right'])!!}
  ...
  <span class="sr-only"> جارى المراجعه</span>
</div>
                    </td>
            @endif
                    ...
                    @if($post->isAccepted())
                    <td>موافق عليه
                    <i class="fa fa-star fa-3x heed spin infinite HD-LightGreen"></i>
                    </td>
            @endif

                     @if($post->isNotAccepted())
                    <td>غير موافق
                    <i class="fa fa-exclamation-triangle fa-3x heed flashy infinite HD-Red"></i>
                    </td>
            @endif
